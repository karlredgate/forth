...
vector: here \ For switching between resident and meta assembling
vector: asm-allot \ For switching between resident and meta assembling
vector: asm@ \ For switching between resident and meta assembling
vector: asm! \ For switching between resident and meta assembling
...
;
: setbits  ( opcode -- )  here  /l asm-allot  asm!  ;
: opaddr  ( -- addr )  here /l - ;
...
.( XXX creating wrpr instructions causes BUSERR ) cr
.( XXX Change fload words so we can exit from the fload ) cr
bye
.( Before BUSERR ) cr
...
.( Before trapif ) cr
: trapif  ( src cond -- )  3a set-op2  addbits  src  ;
...
: ,%icc ( -- ) h# 30.0000 clearbits ;
: ,%xcc ( -- ) ,%icc  h# 20.0000 addbits ;
...
1 bpr: brz     2 bpr: brlez    3 bpr: brnz
